 ...
 *
 * Tuned for Payslip: near-black base + an emerald→teal→cyan ink band (our
 * "verified money" palette) instead of the original cyan→magenta. Runs a dense
 * load burst, then an invisible auto-cursor orbits the centre forever. Returns a
 * destroy() that cancels the RAF loop and removes window listeners (React cleanup).
 */
export function fluidSimulation(canvas) {
    ...
    let config = {
        ...
        COLORFUL: true,
        PAUSED: false,
        BACK_COLOR: { r: 6, g: 10, b: 12 }, // near-black, faint cool undertone
        TRANSPARENT: false,
        BLOOM: false,
        BLOOM_ITERATIONS: 8,
    ...
    function generateColor() {
        // Emerald → teal → cyan band (our "verified money" palette), full brightness.
        const h = 0.38 + Math.random() * 0.16;
