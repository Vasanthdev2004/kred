// Deploy KredEscrow (milestone escrow for invoices) to Arc testnet.
//
...
//   4. Copy the printed address into NEXT_PUBLIC_KRED_ESCROW_ADDRESS
...
const source = fs.readFileSync(
  path.join(__dirname, "../contracts/KredEscrow.sol"),
  "utf8",
);
const input = {
  language: "Solidity",
  sources: { "KredEscrow.sol": { content: source } },
  settings: {
...
const artifact = output.contracts["KredEscrow.sol"].KredEscrow;
...
console.log(`Deploying KredEscrow from ${account.address} → Arc testnet…`);
// KredEscrow only ever holds these two, fixed at deployment. Kept in sync with
// config/arc.ts — an allowlist is what stops a squatter opening a fake-token escrow
// at someone else's invoice id.
const USDC = "0x3600000000000000000000000000000000000000";
const EURC = "0x89B50855Aa3bE2F677cD6303Cec089B5F319D72a";
console.log();
const hash = await wallet.deployContract({ abi, bytecode, args: [USDC, EURC] });
...
// NEXT_PUBLIC_KRED_ESCROW_ADDRESS. A no-code address dark-fails the whole site: viem's
...
if (!code || code === "0x") {
  ...
  console.error("  Do NOT set NEXT_PUBLIC_KRED_ESCROW_ADDRESS to it. Aborting.");
  process.exit(1);
}

console.log(`\n✅ KredEscrow deployed at: ${receipt.contractAddress}`);
...
console.log(`   NEXT_PUBLIC_KRED_ESCROW_ADDRESS=${receipt.contractAddress}`);
